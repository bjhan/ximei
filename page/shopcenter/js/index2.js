...
(function () {
    ...
    submitshuzi();
    function submitshuzi() {
        $.ajax({
            url: CFG.interfaceurl+'/sjzx/publish/counts',
            type: "get",
            timeout: 5000,
            success: function (data) {
                console.log(data);
                $("#yueduliang").html(data.totalViewCount);
                $("#yuezizhognxin").html(data.yzzxViewCount);
                $("#gonglueshuliang").html(data.glViewCount);
                $("#pinglunshu").html(data.yzzxReplyCount);
            },
    ...
    function xitonggonggao() {
        $.ajax({
            url: CFG.interfaceurl+'/sjzx/sys/notice/list?offset=0&limit=10',
            type: "get",
            timeout: 5000,
            success: function (data) {
                console.log(data);
    ...
    function addgonggao(data) {
        var str='<div class="huodongdetail" dataid="'+data.id+'">'
            +'<div class="biankuangbiaoqian"><div class="biankuangbiaoqiandesc">活动</div></div>'
            +'<div class="huodongdesc">'+data.title+'</div>'
            +'<div class="huodongtime">'+data.createTime+'</div>'
            +'</div>';
        $("#gongao").before(str);
    }
